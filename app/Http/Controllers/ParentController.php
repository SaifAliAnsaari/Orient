...
class ParentController extends Controller
{
    ...
    public function get_notif_data(){
        ...
        //if($check->notifications_codes){
            ...
            $check = DB::table('subscribed_notifications')->where('emp_id', Auth::user()->id)->first();
            if($check){
                $this->notif_counts = DB::table('notifications_list as nl')->selectRaw('Count(*) as counts')->whereRaw('notif_to = "'.Auth::user()->id.'" AND id NOT IN (Select notif_id from notification_read_status where emp_id = "'.Auth::user()->id.'") AND code IN ('.DB::table('subscribed_notifications as sn')->selectRaw('GROUP_CONCAT(notification_code_id) as notifications_codes')->whereRaw('web = 1 AND emp_id = '. Auth::user()->id)->first()->notifications_codes.') AND id NOT IN (Select notif_id from notification_read_status where emp_id = "'.Auth::user()->id.'")')->first();
    
                $this->notif_data = DB::table('notifications_list as nl')->selectRaw('id, code, message, customer_id, svr_id, cvr_id, created_at, CASE
                 WHEN customer_id IS NOT NULL THEN (Select company_name from customers where id = nl.customer_id) ELSE "CVR" END AS customer_nameOrCvr')->whereRaw('notif_to =  '.Auth::user()->id.' AND id NOT IN (Select notif_id from notification_read_status where emp_id = "'.Auth::user()->id.'") AND code IN ('.DB::table('subscribed_notifications as sn')->selectRaw('GROUP_CONCAT(notification_code_id) as notifications_codes')->whereRaw('web = 1 AND emp_id = '. Auth::user()->id)->first()->notifications_codes.') AND id NOT IN (Select notif_id from notification_read_status where emp_id = "'.Auth::user()->id.'")')->orderBy('id','DESC')->take(4)->get();
    
    
                 $this->all_notification =  DB::table('notifications_list as nl')->selectRaw('id, code, message, customer_id, svr_id, cvr_id, created_at, Case When customer_id IS Not Null Then (Select name from users where id = (Select created_by from customers where id = nl.customer_id)) Else (Case When cvr_id Is Not Null Then (Select name from users where id = (Select report_created_by from cvr_core where id = nl.cvr_id)) Else (Case When svr_id Is Not Null Then (Select name from users where id = (Select report_created_by from svr_core where id = nl.svr_id)) Else (Select name from users where id = (Select created_by from complains where id = nl.complain_id)) END) END ) END AS customer_nameOrCvr, CASE WHEN svr_id IS NOT NULL THEN (Select company_name from customers where id = (Select customer_visited from svr_core where id = nl.svr_id)) ELSE (Case When cvr_id IS NOT NULL THEN (Select company_name from customers where id = (Select customer_visited from cvr_core where id = nl.cvr_id)) Else "" END ) END AS notif_for, created_at')->whereRaw('notif_to =  '.Auth::user()->id.' AND code IN ('.DB::table('subscribed_notifications as sn')->selectRaw('GROUP_CONCAT(notification_code_id) as notifications_codes')->whereRaw('web = 1 AND emp_id = '. Auth::user()->id)->first()->notifications_codes.')')->orderBy('id','DESC')->get();
            }else{
                $this->notif_counts = 0;
                $this->notif_data = [];
